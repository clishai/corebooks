...
export default function EditAccountModal({ account, onClose, onSaved }: Props) {
  ...
  function handleTypeChange(type: AccountType) {
    setForm((f) => ({
      ...
      classification:
        type === 'Asset' || type === 'Liability' ? (f.classification ?? 'current') : undefined,
    }))
  }
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="relative group">
                  ...
                  <div className="absolute right-0 top-full mt-1 z-20 w-72 hidden group-hover:block bg-raised border border-rim rounded-lg px-3 py-2.5 text-xs text-ash leading-relaxed shadow-xl">
                    {CLASSIFICATION_TOOLTIP[form.type as 'Asset' | 'Liability']}
                  </div>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="flex gap-6">
              {(['debit', 'credit'] as const).map((nb) => (
                <label key={nb} className="flex items-center gap-2 text-sm text-chalk cursor-pointer">
                  <input
                    type="radio"
                    name="normalBalance"
                    value={nb}
